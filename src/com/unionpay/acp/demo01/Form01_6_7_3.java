 ...
 */
public class Form01_6_7_3 extends DemoBase {
	...
	public static Map<String, Object> setFormDate() {
		 ...
		 * 
		 * //商户自定义保留域，交易应答时会原样返回 contentData.put("reqReserved", reqReserved);//O
		 * 
		 * //格式如下：{子域名1=值&子域名2=值&子域名3=值} 移动支付参考消费 contentData.put("reserved",
		 * reserved);//O
		 * 
		 * //渠道类型为语音支付时使用用法见VPC交易信息组合域子域用法 contentData.put("vpcTransData",
		 * vpcTransData);//C
		 * 
		 * //特殊计费用 contentData.put("sndInsCode", sndInsCode);//O
		 */return contentData;
	}
